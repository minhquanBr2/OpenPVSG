...
class LoadOutputsFromMask2Former:
    """Process outputs of all images of a singel video from mask2former to
    inputs of unitracker.
    ...
    def _get_binary_masks_and_query_feats(
            self, pan_mask, query_feat_dict):  # for single frame all objects
        object_ids = list(np.unique(pan_mask))
        if self.num_classes in object_ids:
            object_ids.remove(self.num_classes)
        if len(
                object_ids
        ) == 0:  # deal with the case that no mask was detected from mask2former!!!!
            return np.array([]), []
        assert len(list(query_feat_dict.keys())) == len(
            object_ids), 'Masks and query feats should match!'
        ...
        for object_id in object_ids:
            ...
            query_feats.append(
                dict(query_feat=self._unify_query_feat_dim(
                    query_feat_dict[object_id]),
                     cls_id=cls_id))  # zip the two things together

        return np.stack(
            binary_masks
        ), query_feats  # obs should be numpy array ! not tensor
    ...
    def __getitem__(self, idx):
        ...
        labels, query_feats = self._get_binary_masks_and_query_feats(
            self.pan_masks_all_images[idx],
